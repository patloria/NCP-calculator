...
var events = require('events'), fs = require('fs');
 ...
 * @constructor
 * @param readStream
 *            {ReadStread} An instance of the ReadStream Cl
 * @param options
 *            {Object} optional paramaters for the reader <br/> - separator
 *            {String} - quote {String} - escape {String} - comment {String} -
 *            columnNames {Boolean} - columnsFromHeader {Boolean} - nestedQuotes
 *            {Boolean}
 */
 ...
 * @method parse
 * @param data
 *            {Array} Array of values to parse from the incommin file
 */
	...
	for ( var i = 0; i < data.length; i++) {
		...
		case this.quotechar:
			...
			if (!isEscape && (c === this.quotechar)) {
				...
				if (ps.quotedField) {
					// closing quote should be followed by separator unless the
					// nested quotes option is set
					var nextChar = data.charAt(i + 1);
					if (nextChar && nextChar != '\r' && nextChar != '\n'
							&& nextChar !== this.separator
							&& this.nestedQuotes != true) {
						throw new Error(
								"separator expected after a closing quote; found "
										+ nextChar);
					} else {
		...
		case this.commentchar:
			if (ps.commentedLine)
				break;
			if (ps.openRecord.length === 0 && ps.openField === ''
					&& !ps.quotedField) {
...
CsvReader.prototype.end = function() {
	...
	if (ps.quotedField) {
		this.emit('error', new Error(
				'Input stream ended but closing quotes expected'));
	} else {
		// dump open record
		if (ps.openField) {
			this._addField();
...
CsvWriter.prototype.writeRecord = function(rec) {
	...
	if (!Array.isArray(rec)) {
		throw new Error(
				"CsvWriter.writeRecord only takes an array as an argument");
	}
...
function _appendField(outArr, writer, field) {
	...
	for ( var i = 0; i < field.length; i++) {
		if (field.charAt(i) === writer.quotechar
				|| field.charAt(i) === writer.escapechar) {
